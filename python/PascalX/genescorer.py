...
class cauchy(genescorer):
    ...
    def _scoremain(self,gene,mode='auto',label='',baroffset=0,nobar=False,lock=None):
    ...
    def score(self,gene,parallel=1,unloadRef=False,mode='auto',nobar=False):
        """
        Performs gene scoring for a given list of gene symbols
        ...
        if parallel <= 1:
            R = self._scoremain(G,unloadRef,mode,'',0,nobar,lock)
        else:
            R = [[],[],[]]
            S = np.array_split(G,parallel)
            ...
            for i in range(0,len(S)): 
                ...
                result = pool.apply_async(self._scoremain, (S[i],True,mode,'',i,nobar,lock))
                result_objs.append(result)
